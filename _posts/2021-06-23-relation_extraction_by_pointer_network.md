...
今天读了一篇ACL2021的做实体识别和抽取的[文章](https://arxiv.org/pdf/2010.12812.pdf)，标题为“A Frustratingly Easy Approach for Entity and Relation Extraction”，文章的思路没有辜负“frustratingly”这个词，从实验结果上来看，确实是是简单有效。下面我们先简单回顾一下作者的思路，接着聊一聊对实体标注-关系抽取这个典型任务的一些简单的认识。

<img src="http://qiniu.shihanmax.top/未命名文件.jpg" alt="test" style="zoom: 50%;" />
...
从作者的描述来看，两个encoder应该是分开finetune的，个人觉得两个应该是可以做一个多任务学习，同时训练的，作者的考虑是什么就不得而知了。

从上面分析来看，从理性的角度，我给这几个缺点的影响程度是2>1>3>5>4，但从感性角度来看，则需要把4排在最前面，因为串联意味着复杂，复杂意味着维护、迭代复杂程度高。

这里也提一个稚嫩的想法（可能是看的文献还比较少，目前还没有找到成熟的方案，也可能是有人尝试过了，但效果不行）：

主要思路是，将经典的标注-分类框架变为encoder-decoder架构，我前段时间曾经使用过PointerNet做排序和生成，指针网络可以看做attention机制的简化版本，是否也可以借用它通过解码的方式对实体和关系进行联合抽取呢？
...
<img src="http://qiniu.shihanmax.top/截屏2021-06-24%2020.00.25.png" alt="my_model" style="zoom:67%;" />
