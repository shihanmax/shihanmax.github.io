...
在NNLM中，同样引入了N元的语言模型假设，即一个词出现的概率仅与其前$N-1$个词有关。针对词$w_i$，其上文的$N-1$个词汇为$\{ w_{i-n+1},...,w_{i-1} \}$，使用embedding lookup table：

$$ C_{ \vert V \vert \times m }$$

来获得这些词的隐藏向量，其中，$m$为词向量的维度，$\vert V \vert$表示词库的大小，接着，将上述$N-1$个$1\times m$的表征拼接起来，得到$1 \times m \times (N-1)$的向量$c$，通过公式：
