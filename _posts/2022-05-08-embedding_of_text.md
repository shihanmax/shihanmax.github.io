...
$$P(w_{i}|w_{i-1},w_{i-2}, ...,w_{1})$$

表示当文本段$w_{i-1},w_{i-2}, ...,w_{1}$出现时，下一个词是$w_i$的概率。这种分解形式是一个恒等分解，所以同样会面临稀疏的问题，这时可以引入一个假设，即当前词出现的概率，仅和前面$N-1$个词有关，即仅考虑长度为$N$的窗口，这个假设也称为$N$阶马尔可夫假设，则上述概率分解式可以写为：
