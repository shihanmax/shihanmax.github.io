...
CasRel是发表在ACL2020上的一篇联合抽取模型。文本经过BERT Encoder获得每个token对应的表征$R_{L*H}$之后，首先通过一个span NER层（Subject Tagger，ST），ST包含两个参数矩阵：$W_{start}$，$W_{end}$，维度均为$H*1$。文本表征经过这两个参数投影后，所有的头实体（subject）的首尾位置都被抽取出来。
