...
`visual.conf`：样式自定义。`[labels]`下方为标签的定义，应当和`annotations.txt`中定义的类型相同。这里可以为实体类型、关系类型等设置别名（当标签拥挤，显示重叠时，回自动切换到别名标签）。详细设置可以参考[官方文档](http://brat.nlplab.org/configuration.html#visual-configuration)。
