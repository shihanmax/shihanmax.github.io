...
定义$A$为状态转移矩阵：{ % math % } $A=[ a_{ ij } ]_{N\times N}$ { % endmath % }，其中$a_{ij}$表示在$t$时刻状态为$q_i$时，$t+1$时刻转移到状态$q_j$的概率。
...
设$\theta= \{ \theta_{k} \} \in \Re^{K}$为参数向量，$\mathcal{T}= \{ t_{k} (y, y', \mathbf{x}_{t} ) \}_{k=1}^{K}$，$\mathcal{S}= \{s_{l} (y, \mathbf{x}_{t} ) \}_{l=1}^{L}$分别为定义在边上和定义在节点上的实值特征函数集合，则线性链条件随机场的条件概率有如下形式：
