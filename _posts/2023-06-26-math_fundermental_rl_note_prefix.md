...
在理解actor-critic架构时，有很多直观的解释，其中我最喜欢的是用数学的式子来解释。这个式子用于更新策略的参数$\theta$，通过逐渐调整$\theta$，策略会逐渐变得更好。所以，这样一个式子或算法对应的就是actor。在更新时，我们需要求出值$q$，然后根据这个值来更新策略。这就是策略和值之间的交互关系。
